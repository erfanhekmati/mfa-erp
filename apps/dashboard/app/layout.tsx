 ...
 */
const THEME_INIT_SCRIPT = `(function(){try{var k=${JSON.stringify(THEME_STORAGE_KEY)};var v=localStorage.getItem(k);var a=${JSON.stringify(themeIds)};var t=${JSON.stringify(defaultThemeId)};if(v&&a.indexOf(v)!==-1)t=v;document.documentElement.dataset.theme=t;var def=${JSON.stringify(defaultThemeId)};if(t===def){var p=location.pathname||"/";if(p.indexOf("/settings")===0||p.indexOf("/profile")===0)document.documentElement.dataset.section="settings";else if(p.indexOf("/sales")===0)document.documentElement.dataset.section="sales";else if(p.indexOf("/warehouse")===0)document.documentElement.dataset.section="warehouse";else if(p.indexOf("/inventory/products")===0||p.indexOf("/inventory/product-groups")===0)document.documentElement.dataset.section="base-info";else if(p.indexOf("/inventory")===0)document.documentElement.dataset.section="inventory";else if(p.indexOf("/customers")===0)document.documentElement.dataset.section="base-info";else if(p.indexOf("/reports")===0)document.documentElement.dataset.section="reports";else document.documentElement.removeAttribute("data-section");}else document.documentElement.removeAttribute("data-section");}catch(e){}})();`;
